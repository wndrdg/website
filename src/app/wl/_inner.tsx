...
export function WaitlistInviteInner({
  initialInviteCode,
  initialCodeDescription,
  initialCodeValid,
}: {
  initialInviteCode: string;
  initialCodeDescription: string | null;
  initialCodeValid: boolean | null;
}) {
  // Invite code lookup happens server-side (see ./page.tsx). The initial
  // values arrive baked into the rendered HTML so the golden ticket appears
  // with the page itself — no client fetch round-trip, no "Verifying invite…"
  // flash on bad wifi.
  const inviteCode = initialInviteCode;
  const [codeDescription] = useState<string | null>(initialCodeDescription);
  const [codeLoading] = useState<boolean>(false);
  const [codeValid] = useState<boolean | null>(initialCodeValid);
